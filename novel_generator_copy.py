...
class Judge(BaseModel):
    quality: Literal["good", "not good"] = Field(
        description="judge quality for this chapter"
    )


judger = model1.with_structured_output(Judge)
...
def create_structured_evaluator_prompt(text):
    return (
        ...
        "{\"quality\": \"good\" or \"bad\"}\n\n"
        f"Evaluate this chapter: {text}\n"
        "Remember to ONLY respond with the JSON object, nothing else."
    )

# Set up a parser + inject instructions into the prompt template.
parser = PydanticOutputParser(pydantic_object=Judge)
...
def generate_outline_node(state: NovelWorkflowState) -> NovelWorkflowState:
    ...
    if chapter_number == 1:
        ...
        premise_res = model1.invoke(messages)
        ...
        outline_res = model1.invoke(messages)
...
def judge_quality_node(state: NovelWorkflowState) -> NovelWorkflowState:
    chapter_number = state["current_chapter_number"]
    judge_prompt = create_structured_evaluator_prompt(state["current_chapter_full_content"])
    messages = [
        SystemMessage(
            f"你是一个多章节小说的质量评估师。当前正在评估第 {chapter_number} 章的质量。\n"
            f"小说的主题大纲是：\n{state['novel_premise']}\n\n"
            f"上一章节的内容是：\n{state['previous_chapter_content']}\n\n"
            "请根据以上所有信息，综合评估本章内容的质量。考量因素包括：是否符合大纲、与前文的连贯性、情节是否吸引人、描写是否生动等。"
            "如果质量良好，请输出 'good'。如果质量不佳，需要重写，请输出 'not good'。"
            "请直接输出 'good' 或 'not good'。不要包含其他额外说明。"
        ),
        HumanMessage(
    ...
    print(f"第{chapter_number}章的质量评估：{judge_res.quality}")
    print(f"第 {chapter_number} 章质量评估：{judge_res.quality}")
    return {"decision": judge_res.quality}
...
def write_chapter_node(state: NovelWorkflowState) -> NovelWorkflowState:
    ...
    system_prompt += (
        ...
        "确保每一章字数不超过两千字，且确保每次只写一章内容并且写完整，不能有遗漏。请你务必完整的输出给我"
    )
    ...
    chapter_content_res = model2.invoke(messages)
